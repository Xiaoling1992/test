...
function find_file_or_dir_in_project {
  ...
  if [[ ! -e "${proj_path}/${file_or_dir_name}" ]]; then
    echo -e "[${color_red}FAIL${color_norm}]: no ${file_or_dir_name} found"   #change 2: don't print missing information
...
function setup_testing_input_output {
  ...
  if [ -d ${TEST_OUTPUT_PATH} ]; then
    rm -rf ${TEST_OUTPUT_PATH}  #delete the temp folder if it exists already.
  fi
  ...
  rm -r ${TEST_OUTPUT_PATH}/input/*   #delete the input and output files in it.
  rm -r ${TEST_OUTPUT_PATH}/output/*
  cp -r ${GRADER_ROOT}/tests/${test_folder}/input/order_products.csv ${TEST_OUTPUT_PATH}/input/order_products.csv  #copy the files in test cases to the temp folder.
...
function compare_outputs {
  ...
  DIFF_RESULT1=$(diff -bB ${PROJECT_ANSWER_PATH1} ${TEST_ANSWER_PATH1} | wc -l)    #How to evalute the test result: compare test result with the project output.
  if [ "${DIFF_RESULT1}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH1} ]; then
    #echo -e "[${color_green}PASS${color_norm}]: ${test_folder} ${OUTPUT_FILENAME}"
    echo -e "[${color_green}PASS${color_norm}]: ${test_folder}"   #change 1. output 
    echo "[$(date)] 1 of 1 tests passed"
    NUM_OUTPUT_FILES_PASSED=$(($NUM_OUTPUT_FILES_PASSED+1))
  else
    echo -e "[${color_red}FAIL${color_norm}]: ${test_folder}"
    echo "[$(date)] 0 of 1 tests passed"
